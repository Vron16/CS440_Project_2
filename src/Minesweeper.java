...
//                    System.out.println("NumErrors: " + numErrors);
                    directInference(knowledgeBaseBoard, null, kbQuery, availableCells, clearedCells, constraints);
                }
                //constraintSatisfy
...
//                      } else {
//                          directInference(knowledgeBaseBoard, null, knowledgeBaseBoard[contradiction.row][contradiction.col],
...
//                String keepGoing = reader.nextLine();
                ...
                BoardNode kbQuery = availableCells.remove(randGenerator.nextInt(availableCells.size()));
                int clue = board.query(kbQuery.row, kbQuery.col);
//                System.out.println("Querying cell: (" + kbQuery.row + ", " + kbQuery.col + ") with mine prb " + kbQuery.minePrb + " and clue: " + clue);
                numErrors += updateQuery(knowledgeBaseBoard, null, kbQuery, clue, constraints);
//                System.out.println("NumErrors: " + numErrors);
                directInference(knowledgeBaseBoard, null, kbQuery, availableCells, clearedCells, constraints);
            }
...
//        return (totalMines - numErrors)/(totalMines);
    ...
    public static double playInferenceProbGame(Environment board, double totalMines, boolean isMinCost) {
        double numErrors = 0;
        double guesses = 0;
        BoardNode[][] knowledgeBaseBoard = new BoardNode[board.getDim()][board.getDim()];
        for (int i = 0; i < board.getDim(); i++) {
...
//        	return guesses;
    ...
    public static double playImprovedGame(Environment board, double totalMines, int[] difMinCost, int[] difMinRisk, int[] guesses) {
        double numErrors = 0;
        BoardNode[][] knowledgeBaseBoard = new BoardNode[board.getDim()][board.getDim()];
        for (int i = 0; i < board.getDim(); i++) {
            for (int j = 0; j < board.getDim(); j++) {
...
//                String keepGoing = reader.nextLine();
                assignMinePrbs(knowledgeBaseBoard, constraints);
         	    guesses[0]++;
         	    BoardNode kbQuery = new BoardNode (0, 0);
         	    kbQuery = availableCells.get(cellWithMinMinePrb(availableCells));
         	    boolean tie = checkForTie (kbQuery, availableCells);
         	    if (!tie) {
         	    	difMinRisk[0]++;
         	    	availableCells.remove(kbQuery);
         	    }
         	    	...
         	    	difMinCost[0]++;
                    computeRisk(knowledgeBaseBoard, constraints);
                    ArrayList <BoardNode> chooseFromTies = new ArrayList <BoardNode> ();
                    for (int i = 0; i < availableCells.size(); i++) {
                    	if (kbQuery.minePrb == availableCells.get(i).minePrb) {
                    		chooseFromTies.add(availableCells.get(i));
                    	}
                    }
                    kbQuery = availableCells.remove(cellWithMinRisk(chooseFromTies));
         	    }
                int clue = board.query(kbQuery.row, kbQuery.col);
                //System.out.println("Querying cell: (" + kbQuery.row + ", " + kbQuery.col + ") with mine prb " + kbQuery.minePrb);
                numErrors += updateQuery(knowledgeBaseBoard, null, kbQuery, clue, constraints);
...
//        	return guesses;
    ...
    public static boolean checkForTie (BoardNode kbQuery, ArrayList<BoardNode>available) {
    	for (int i = 0; i < available.size(); i++) {
    		if (available.get(i) != kbQuery && available.get(i).minePrb == kbQuery.minePrb) {
    			return true;
    		}
    	}
    	return false;
...
//          System.out.println("Incrementing numErrors.");
            numErrors++;
...
//              System.out.print("The following constraint results in a contradiction: ");
                ...
                if (equation.length() > 0) {
                    ...
                    equation.append("0 = " + currentConstraint.minesLeft);
                }
...
//                                  + ") = " + minesLeftDiff);
                        ...
                        if (minesLeftDiff >= 0 && minesLeftDiff <= varsNotInTwo.size()) {
                            ...
                            if (equation.length() > 0) {
                                ...
                                equation.append("0 = " + minesLeftDiff);
                            }
                            equation.append(" = " + minesLeftDiff);
//                          System.out.println(equation.toString());
                            break;
...
//  }
    ...
    public static DefaultXYDataset agentPerformance() {
        ...
        double dim = 7.0;
        int trial = 500;
        int[] guesses = new int [1]; guesses[0] = 0;
        int[] difMinCost = new int [1]; difMinCost[0] = 0;
        int[] difMinRisk = new int [1]; difMinRisk[0] = 0;
        double [][] randomScores = new double [2][(int) Math.pow(dim, 2.0)];
        double [][] minCostScores = new double[2][(int) Math.pow(dim, 2.0)];
        double [][] minRiskScores = new double [2][(int) Math.pow(dim, 2.0)];
        double [][] improvedScores = new double[2][(int) Math.pow(dim, 2.0)];
        for (int numMines = 1; numMines <= (int) Math.pow(dim, 2.0); numMines++) {
//            double totalBasicScore = 0.0;
//            double totalDirInfScore = 0.0;
        	double totalRandomErrors = 0.0;
        	double totalMinCostErrors = 0.0;
        	double totalMinRiskErrors = 0.0;
        	double totalImprovedErrors = 0.0;
            for (int i = 0; i < trial; i++) {
                Environment myBoard = new Environment((int)dim, numMines);
                totalRandomErrors += playGame (myBoard, numMines);
                totalMinCostErrors += playInferenceProbGame(myBoard, numMines, true);
                totalMinRiskErrors += playInferenceProbGame(myBoard, numMines, false);
                totalImprovedErrors += playImprovedGame(myBoard, numMines, difMinCost, difMinRisk, guesses);
            }
//            double avgBasicScore = totalBasicScore/trial;
//            double avgDirInfScore = totalDirInfScore/trial;
            double avgRandomErrors = totalRandomErrors/trial;
            double avgMinCostErrors = totalMinCostErrors/trial;
            double avgMinRiskErrors = totalMinRiskErrors/trial;
            double avgImprovedErrors = totalImprovedErrors/trial;
            System.out.println("Mine Density: " + numMines/Math.pow(dim, 2.0));
//            System.out.println("Avg Min Cost Errors: " + avgMinCostErrors);
//            System.out.println("Avg Min Risk Errors: " + avgMinRiskErrors);
            randomScores[0][numMines-1] = numMines/Math.pow(dim, 2.0);
            randomScores[1][numMines-1] = avgRandomErrors;
            minCostScores[0][numMines-1] = numMines/Math.pow(dim, 2.0);
            minCostScores[1][numMines-1] = avgMinCostErrors;
            minRiskScores[0][numMines-1] = numMines/Math.pow(dim, 2.0);
            minRiskScores[1][numMines-1] = avgMinRiskErrors;
            improvedScores[0][numMines-1] = numMines/Math.pow(dim, 2.0);
            improvedScores[1][numMines-1] = avgImprovedErrors;
        }
       
        scoreData.addSeries("Random Guesses vs. Mine Density", randomScores);
        scoreData.addSeries("Minimizing Cost vs. Mine Density", minCostScores);
        scoreData.addSeries("Minimizing Risk vs. Mine Density", minRiskScores);
        scoreData.addSeries("Improved Agent vs. Mine Density", improvedScores);
        System.out.println("number of times improved agent had to guess = " + guesses[0]);
        System.out.println("number of times improved made a different decision from minCost = " + difMinCost[0]);
        System.out.println("number of times improved made a different decision from minRisk = " + difMinRisk[0]);
        return scoreData;
...
//      Environment gameBoard = new Environment(10, 15);
//    
//      BoardCellPanel[][] mineSweeperCells = buildBoard(10);
...
//          }
//      }
//      double score = playGame(gameBoard, 15);
//      System.out.println("Basic Agent Score: " + score);
//      score = playInferenceGame(gameBoard, 50);
